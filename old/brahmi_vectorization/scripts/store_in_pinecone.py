...
for word, data in embeddings_dict.items():
    word_id = hashlib.md5(word.encode("utf-8")).hexdigest()      # Generate a unique ASCII ID by hashing the Brahmi word
    vector = data["vector"]  # Already a list, no need to convert
    metadata = {
        "Brahmi_word": word,
    ...
    index.upsert([(word_id, vector, metadata)])
