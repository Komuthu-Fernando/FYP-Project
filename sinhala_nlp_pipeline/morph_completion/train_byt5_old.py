...
# CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
# MODEL_DIR = os.path.join(CURRENT_DIR, '..', 'models', 'byt5_sinhala', 'final')
# DATA_FILE = os.path.join(CURRENT_DIR, '..', 'data', 'morph_train_new.txt')
# LOG_FILE = os.path.join(CURRENT_DIR, '..', 'logs', 'training_log.txt')
...
def train_byt5():
    ...
    for epoch in range(epochs):
        ...
        save_path = os.path.join(MODEL_DIR, f'epoch_{epoch+1}')
        os.makedirs(save_path, exist_ok=True)
        model.save_pretrained(save_path)
        tokenizer.save_pretrained(save_path)
        log_to_file(f"Model saved to {save_path}")

        model.train()

    log_to_file(f"Training completed. Final model saved to {MODEL_DIR}")
