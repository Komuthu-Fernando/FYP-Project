...
# from torch.optim import AdamW  # Updated import from torch.optim
...
# def prepare_data():
#     data = []
#     with open(os.path.join(CURRENT_DIR, '..', 'data', 'morph_train.txt'), 'r', encoding='utf-8') as f:
#         for line in f:
#             line = line.strip()
#             if line:
#                 parts = line.split(' → ')
...
#     optimizer = AdamW(model.parameters(), lr=5e-5)
#     scheduler = StepLR(optimizer, step_size=1, gamma=0.9)  # Reduce LR by 10% each epoch
...
#         inputs = [f"input: {item['input']} → output:" for item in batch]
#         targets = [item['output'] for item in batch]
#         input_enc = tokenizer(inputs, padding=True, truncation=True, max_length=50, return_tensors='pt')
#         target_enc = tokenizer(targets, padding=True, truncation=True, max_length=50, return_tensors='pt')
#         return {'input_ids': input_enc['input_ids'], 'labels': target_enc['input_ids']}
